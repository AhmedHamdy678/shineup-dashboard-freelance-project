    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden relative">
          ...
          {logoUrl ? (
            <img
              ...
              onError={(e) => {
                ...
                e.target.className =
                  "w-10 h-10 object-contain text-emerald-600 opacity-50";
              }}
            />
          ) : (
      ...
      <div className="pt-12 p-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex flex-col items-start">
            <h2 className="text-xl font-bold text-gray-900">
              {nameBusiness || "الاسم التجاري"}
            </h2>
            {nameBusinessEn && (
              <h3
                className="text-sm font-medium text-gray-500 mt-0.5 text-left"
                dir="ltr"
              >
                {nameBusinessEn}
              </h3>
          ...
          {approvalStatus && (
            <span
              className={`px-2 py-0.5 text-xs font-medium rounded-full shrink-0 ${isApproved ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
            >
        ...
        <div className="flex flex-col items-center text-center gap-2 mt-4 w-full">
          ...
          {descriptionEn && (
            <div
              className="text-gray-600 text-sm whitespace-pre-wrap border-t border-gray-100 pt-3 mt-1 w-full"
              dir="ltr"
            >
              {descriptionEn}
            </div>
