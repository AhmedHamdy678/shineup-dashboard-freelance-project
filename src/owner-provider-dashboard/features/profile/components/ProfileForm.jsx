        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <div
                className="w-full px-3 py-2 border border-gray-100 bg-gray-50 text-gray-500 rounded-lg text-sm cursor-not-allowed flex items-center justify-between"
                dir="ltr"
              >
                <span className="truncate">
                  {profile?.owner?.phone || profile?.phone || "---"}
                </span>
                <span className="text-[10px] text-gray-400 font-medium shrink-0 bg-gray-200 px-1.5 py-0.5 rounded">
                  غير قابل للتعديل
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 mt-4">
          <div className="flex flex-col items-start gap-1 w-full text-right">
            ...
            <span className="text-base font-semibold text-gray-900">
              {profile?.owner?.fullName || profile?.fullName || "---"}
            </span>
          </div>
          <div className="flex flex-col items-start gap-1 w-full text-right">
            <span className="text-sm font-medium text-gray-500">
            ...
            <span
              className="text-base font-semibold text-gray-900 text-left w-auto max-w-full"
              dir="ltr"
            >
              {profile?.owner?.phone || profile?.phone || "---"}
            </span>
          ...
          <div className="flex flex-col items-start gap-1 w-full text-right">
            ...
            <span
              ...
              dir="ltr"
            >
              {profile?.owner?.email || profile?.email || "---"}
            </span>
          </div>
          <div className="flex flex-col items-start gap-1 w-full text-right">
        ...
        {isEditMode ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              ...
              />
              {errors.nameBusinessAr && (
                <p className="text-red-500 text-xs mt-1">
                  {errors.nameBusinessAr.message}
                </p>
              )}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                الاسم التجاري (إنجليزي) / Business Name (EN)
              </label>
              <input
                type="text"
              ...
              />
              {errors.nameBusinessEn && (
                <p className="text-red-500 text-xs mt-1">
                  {errors.nameBusinessEn.message}
                </p>
              )}
            </div>
            <div>
            ...
            <div className="flex flex-col items-start gap-1 w-full text-right">
              ...
              <div className="flex flex-col items-start gap-1 w-full text-right">
                <span className="text-base font-semibold text-gray-900 block">
                  {profile?.nameBusinessAr || profile?.nameBusiness || "---"}
                </span>
                {profile?.nameBusinessEn && (
                  <span
                    className="text-sm text-gray-500 block text-left w-auto max-w-full"
                    dir="ltr"
                  >
                    {profile.nameBusinessEn}
                  </span>
          ...
          {isEditMode ? (
            ...
            <div className="flex flex-col gap-4 mt-4">
              <div className="flex flex-col items-start gap-1 w-full text-right">
                ...
                <span className="text-base font-semibold text-gray-900 whitespace-pre-wrap">
                  {profile?.descriptionAr || profile?.description || "---"}
                </span>
              </div>
              <div className="flex flex-col items-start gap-1 w-full text-right">
                ...
                <span
                  dir="ltr"
                  className="text-base text-gray-900 text-left w-auto max-w-full whitespace-pre-wrap"
                >
...
function FileUploadZone({
    ...
    if (!file && existingUrl) {
      let urlStr =
        typeof existingUrl === "string"
          ? existingUrl
          : existingUrl?.downloadUrl ||
            existingUrl?.url ||
            existingUrl?.path ||
            (existingUrl?.mediaId ? `/media/${existingUrl.mediaId}` : "");

      if (urlStr && urlStr.includes("localhost")) {
        const baseUrl =
          import.meta.env.VITE_SOCKET_URL || "https://api.example-company.tech";
        urlStr = urlStr.replace(/^https?:\/\/localhost(:\d+)?/, baseUrl);
      }
      const isImage =
        name === "logoFile" ||
        name === "coverFile" ||
        (urlStr && urlStr.match(/\.(jpeg|jpg|gif|png|webp)$/i));
        ...
        if (urlStr.startsWith("http") || urlStr.startsWith("data:")) {
          ...
          const fetchImage = async () => {
            try {
              ...
              const res = await providerAxiosClient.get(cleanPath, {
                responseType: "blob",
              });
              if (isMounted) {
  ...
  if (!file && existingUrl) {
    ...
    const handleDownload = async (e, path) => {
      ...
      if (!path) {
        const dbg =
          typeof existingUrl === "object"
            ? JSON.stringify(existingUrl)
            : String(existingUrl);
        toast.error(`بيانات الملف: ${dbg.substring(0, 100)}`, {
          duration: 6000,
        });
        return;
      }
      ...
      try {
        ...
        if (status === 404) {
          toast.error(
            "عذراً، لم يتم العثور على الملف في الخادم. قد يكون محمي أو تم حذفه.",
          );
        } else {
    ...
    return (
      <div className="border border-gray-200 bg-white rounded-xl p-4 flex flex-col items-center justify-center relative group min-h-[120px]">
        ...
        <p
          className="text-xs font-medium text-gray-700 truncate w-full text-center mb-1"
          dir="ltr"
          title={
            typeof existingUrl === "object" &&
