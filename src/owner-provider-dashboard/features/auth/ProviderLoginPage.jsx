...
export default function ProviderLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mobile === "01000000000" && password === "dummy-password-000") {
      login(
        {
          id: "owner_1",
          name: "Ahmed Khalil",
          companyName: "Shine & Co. Detailing",
        },
        "dummy-jwt-token-123",
      );
      navigate("/provider", { replace: true });
    ...
    try {
      const { providerLogin } =
        await import("../../api/endpoints/providerAuth.api");
      ...
      const hasAccess =
        roleNames.some((role) => allowedProviderRoles.includes(role)) ||
        allowedProviderRoles.includes(directRole);

      if (!hasAccess) {
        const detectedRoles =
          roleNames.length > 0
            ? roleNames.join(", ")
            : directRole || "بدون صلاحية";
        setError(
          `عذراً، الصلاحيات الحالية (${detectedRoles}) غير كافية للدخول. الرجاء التواصل مع الدعم.`,
        );
        setLoading(false);
        return;
      }
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-white text-center px-12">
            ...
            <p className="text-white/60 mt-2 max-w-md">
              قم بإدارة أعمالك، الحجوزات، الفريق، والأرباح من مكان واحد
            </p>
          </div>
        </div>
      </div>
