...
export default function StaffActivationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axiosClient.post('/auth/staff/activate', {
      ...
      if (data.success && data.requiresLogin) {
        toast.success('تم تفعيل الحساب بنجاح، يرجى تسجيل الدخول.');
        navigate('/login', { replace: true });
      } else {
        // Fallback for success without specific flags
        toast.success('تم تفعيل الحساب بنجاح.');
        navigate('/login', { replace: true });
      }
    } catch (err) {
      setError(err?.response?.data?.message || 'فشل تفعيل الحساب. يرجى التحقق من الرمز المدخل والمحاولة مرة أخرى.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:block w-1/2 relative overflow-hidden">
        <img
          src={loginImage}
          alt="ShineUp Activation"
          className="absolute inset-0 w-full h-full object-cover"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
        <div className="absolute bottom-10 left-10 text-white">
          <h2 className="text-3xl font-bold">ShineUp</h2>
          <p className="text-lg text-white/80 mt-2">تفعيل حساب المشرف</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12 bg-gray-50">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl shadow-gray-200/50 p-8 border border-gray-100">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-gray-500 mt-1.5">أدخل رمز التحقق (OTP) وقم بتعيين كلمة المرور الجديدة</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">رمز التحقق (OTP)</label>
              <input
                type="text"
                value={otp}
                ...
                required
                dir="ltr"
                className="w-full px-4 py-2.5 bg-gray-50/50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition text-center tracking-widest text-lg"
                placeholder="123456"
                maxLength={6}
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 mt-4 text-sm font-bold text-white bg-blue-600 rounded-xl shadow-md shadow-blue-500/20 hover:bg-blue-700 hover:shadow-lg disabled:opacity-50 transition-all duration-200"
            >
